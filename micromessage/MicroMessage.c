...
status_t UMAddStrings(UMessage * msg, const char * fieldName, const char ** strings, uint32 numStrings)
{
   ...
   uint32 i;
   for (i=0; i<numStrings; i++) numDataBytes += strings[i] ? strlen(strings[i]) : 0;
   ...
   {
      uint32 stringFieldLength = (strings[i]?strlen(strings[i]):0) + 1;
...
{
   uint32 fieldNameLen = strlen(fieldName)+1;
